Feature: User Login
  ...
  Scenario Outline: Correct credentials should permit login
    When    the user provides correct username "<username>" for login
    And     the user provides correct password "<password>" for login
    And     when the user clicks the log button
  ...
  Scenario Outline: Incorrect credentials should prevent login
    When    the user provides incorrect username "<username>" for login
    And     the user provides incorrect password "<password>" for login
    ...
    Examples:
      ...
      |Batman                                |Iamthenight1939   |Invalid Credentials         |